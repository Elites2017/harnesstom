            ...
            foreach ($sheetData as $key => $row) {
                ...
                if ($ontology_id != null && $name != null) {
                    ...
                    if (!$existingAnnotationLevel) {
                        ...
                        $annotationLOntologyIdParentTerm = $entmanager->getRepository(AnnotationLevel::class)->findOneBy(['ontology_id' => $parentTerm]);
                        if (($annotationLOntologyIdParentTerm != null) && ($annotationLOntologyIdParentTerm instanceof \App\Entity\AnnotationLevel)) {
                            $annotationLevel->setParentTerm($annotationLOntologyIdParentTerm);
                        }
                        if ($this->getUser()) {
                            $annotationLevel->setCreatedBy($this->getUser());
                        }
                        ...
                        $annotationLevel->setParentTerm($parentTerm);
                        $annotationLevel->setIsActive(true);
                        $annotationLevel->setCreatedAt(new \DateTime());
                        $entmanager->persist($annotationLevel);
                    }
