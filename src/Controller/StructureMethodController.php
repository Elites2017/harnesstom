...
class StructureMethodController extends AbstractController
{
    ...
    public function uploadFromExcel(Request $request, EntityManagerInterface $entmanager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $repoStructureMethod = $entmanager->getRepository(StructureMethod::class);
            // Query how many rows are there in the structureMethod table
            $totalStructureMethodBefore = $repoStructureMethod->createQueryBuilder('tab')
            ...
            foreach ($sheetData as $key => $row) {
                ...
                if ($ontology_id != null && $name != null) {
                    ...
                    $existingStructureMethod = $entmanager->getRepository(StructureMethod::class)->findOneBy(['ontology_id' => $ontology_id]);
                    // upload data only for countries that haven't been saved in the database
                    if (!$existingStructureMethod) {
                        $structureMethod = new StructureMethod();
                        if ($this->getUser()) {
                            $structureMethod->setCreatedBy($this->getUser());
                        }
                        $structureMethod->setOntologyId($ontology_id);
                        $structureMethod->setName($name);
                        if ($description != null) {
                            $structureMethod->setDescription($description);
                        }
                        if ($parentTermString != null) {
                            $structureMethod->setParOnt($parentTermString);
                        }
                        $structureMethod->setIsActive(true);
                        $structureMethod->setCreatedAt(new \DateTime());
            ...
            foreach ($sheetData as $key => $row) {
                ...
                if ($ontology_id != null && $parentTerm != null ) {
                    // check if the data is upload in the database
                    $ontologyIdParentTerm = $entmanager->getRepository(StructureMethod::class)->findOneBy(['ontology_id' => $parentTerm]);
                    if (($ontologyIdParentTerm != null) && ($ontologyIdParentTerm instanceof \App\Entity\structureMethod)) {
                        $ontId = $ontologyIdParentTerm->getId();
                        // get the real string (parOnt) parent term or its line id so that to do the link 
                        $stringParentTerm = $entmanager->getRepository(StructureMethod::class)->findOneBy(['par_ont' => $parentTerm, 'is_poau' => null]);
                        ...
                        $res = $connexion->executeStatement('UPDATE structure_method SET parent_term_id = ?, is_poau = ? WHERE id = ?', [$ontId, 1, $parentTermId]);
                    }
                }
            }

            // Query how many rows are there in the Country table
            $totalStructureMethodAfter = $repoStructureMethod->createQueryBuilder('tab')
                // Filter by some parameter if you want
                // ->where('a.isActive = 1')
                ->select('count(tab.id)')
                ->getQuery()
            ...
            if ($totalStructureMethodBefore == 0) {
                $this->addFlash('success', $totalStructureMethodAfter . " structure methods have been successfuly added");
            } else {
                $diffBeforeAndAfter = $totalStructureMethodAfter - $totalStructureMethodBefore;
                if ($diffBeforeAndAfter == 0) {
                    $this->addFlash('success', "No new structure method has been added");
                } else if ($diffBeforeAndAfter == 1) {
                    $this->addFlash('success', $diffBeforeAndAfter . " structure method has been successfuly added");
                } else {
                    $this->addFlash('success', $diffBeforeAndAfter . " structure methods have been successfuly added");
                }
            }
            return $this->redirect($this->generateUrl('structure_method_index'));
        }
        ...
        $context = [
            'title' => 'Structure Method Upload From Excel',
            'structureMethodUploadFromExcelForm' => $form->createView()
        ];
        return $this->render('structure_method/upload_from_excel.html.twig', $context);
    }
    ...
    {
        $response = new BinaryFileResponse('../public/todownload/structure_method_template_example.xls');
        $response->setContentDisposition(ResponseHeaderBag::DISPOSITION_ATTACHMENT, 'structure_method_template_example.xls');
        return $response;
