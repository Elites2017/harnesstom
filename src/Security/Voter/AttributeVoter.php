...
class AttributeVoter extends Voter
{
    ...
    public const DEL = 'attribute_delete';
    ...
    protected function supports(string $attribute, $attr): bool
    {
        ...
        return in_array($attribute, [self::EDIT, self::DEL])
            && $attr instanceof \App\Entity\Attribute;
    }

    protected function voteOnAttribute(string $attribute, $attr, TokenInterface $token): bool
    {
        $user = $token->getUser();
        ...
        switch ($attribute) {
            case self::EDIT:
                ...
                return $this->canEdit($attr, $user);
                break;
            case self::DEL:
                // logic to determine if the user can VIEW
                // return true or false
                return $this->canDelete($attr, $user);
                break;
        }
     ...
     // private methods
     private function canEdit(Attribute $attr, User $user){
        // if the connected user is the owner of the object, they can modify it
        return $user === $attr->getCreatedBy();
    }

    private function canDelete(Attribute $attr, User $user){
